...
import com.example.plantbuddy.NGO.details.GetAllNgoState
...
import com.example.plantbuddy.screens.PlantCard
...
fun CommunityScreen(
    ...
    LaunchedEffect(Unit) {
        ...
        campaignViewmodel.getActiveCampaign(true  , null)

    }

    val getAllNgoState by viewModel.getAllNgoState.collectAsState()
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            ...
            LazyColumn(
                ...
                when(val state=getAllNgoState){
                    is GetAllNgoState.Idle -> {
                        item {
                            Text("Idle")
                        }
                    }
                    is GetAllNgoState.Loading -> {
                        item {
                            Text("Loading")
                        }
                    }
                    is GetAllNgoState.Success -> {

                            val previewNgos = state.data.take(10) // show only first 5
                            ...
                            item {
                                Column {
                                    ...
                                    LazyRow(
                                        ...
                                        items(previewNgos) { ngo ->
                                            NgoInfoCard(
                                                name = ngo.name,
                                                city = ngo.city,
                                                logo = ngo.logo.toString(),
                                                onClick = {
                                                    mainNavController.navigate("ngo_detail/${ngo.id}")
                                                }
                ...
                when(val activeCampaignState=getActiveCampaignState){
                    ...
                    is GetActiveCampaignState.Success -> {
                        ...
                        items(
                            items = activeCampaignState.data,
                            key = { it.id }
                        ){
                            ActiveCampaignCard(
